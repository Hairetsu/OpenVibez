...
import type { RunnerContext, RunnerEvent, RunnerHistoryMessage, RunnerResult } from './types';

const TOOL_CALL_PREFIX = 'TOOL_CALL';
const PLAN_PREFIX = 'PLAN';
const STEP_DONE_PREFIX = 'STEP_DONE';
...
type FinalPayload = { message: string };

type ProtocolCompletionResult = {
  text: string;
  inputTokens?: number;
  outputTokens?: number;
...
type ToolProtocolRunnerInput = {
  ...
  runCompletion: (input: {
    history: RunnerHistoryMessage[];
    signal: AbortSignal;
    onEvent?: (event: { type: 'assistant_delta'; delta: string }) => void;
  }) => Promise<ProtocolCompletionResult>;
};
...
const buildToolSystemPrompt = (input: {
  providerLabel: string;
  accessMode: RunnerContext['accessMode'];
  workspacePath?: string;
  trustLevel?: WorkspaceRow['trust_level'];
}): string => {
  ...
  return [
    `You are OpenVibez ${input.providerLabel} coding assistant with autonomous CLI tool access.`,
    '',
    'You MUST follow this protocol:',
    `1) First response: ${PLAN_PREFIX} {"steps":["step 1","step 2",...]}`,
    `2) During execution: respond with either ${TOOL_CALL_PREFIX} {...} or ${STEP_DONE_PREFIX} {"index":<1-based>,"note":"optional"}`,
    `3) Only when every step is completed: ${FINAL_PREFIX} {"message":"final user response"}`,
    '',
    ...
    `${TOOL_CALL_PREFIX} {"name":"run_shell","arguments":{"command":"<shell command>","cwd":"<optional cwd>"}}`,
    ...
    `Only return ${FINAL_PREFIX} after all planned steps are complete.`,
...
const sanitizePlanSteps = (steps: string[]): string[] => {
  ...
  return cleaned.slice(0, MAX_PLAN_STEPS);
};

const parsePlan = (text: string): ExecutionPlan | null => {
  const payload = parsePrefixedJson(text, PLAN_PREFIX) as { steps?: unknown } | null;
...
const parseToolCall = (text: string): ProtocolToolCall | null => {
  const payload = parsePrefixedJson(text, TOOL_CALL_PREFIX) as {
    name?: unknown;
  ...
  const cwd =
    typeof payload.arguments?.cwd === 'string' && payload.arguments.cwd.trim()
...
const parseStepDone = (text: string): StepDone | null => {
  const payload = parsePrefixedJson(text, STEP_DONE_PREFIX) as { index?: unknown; note?: unknown } | null;
  if (!payload) {
    return null;
  }
...
const parseFinal = (text: string): FinalPayload | null => {
  const payload = parsePrefixedJson(text, FINAL_PREFIX) as { message?: unknown } | null;
  if (!payload) {
    return null;
  }
...
const createFinalMessageDeltaParser = (): {
  ...
  return {
    push: (chunk: string): string => {
      ...
      while (true) {
        if (!finalSeen) {
          const finalIndex = buffer.indexOf(FINAL_PREFIX, scanIndex);
          if (finalIndex === -1) {
          ...
          finalSeen = true;
          scanIndex = finalIndex + FINAL_PREFIX.length;
        }
...
export const runToolProtocolAgent = async (input: ToolProtocolRunnerInput): Promise<RunnerResult> => {
  const workspacePath = input.workspace?.root_path;
  const toolSystemMessage = buildToolSystemPrompt({
  ...
  const agentHistory: RunnerHistoryMessage[] = [{ role: 'system', content: toolSystemMessage }, ...input.history];
  let totalInputTokens = 0;
  let totalOutputTokens = 0;
  let totalCostMicrounits = 0;
  ...
  for (let attempt = 0; attempt < 2 && !plan; attempt += 1) {
    ...
    const planningTurn = await input.runCompletion({
    ...
    if (typeof planningTurn.costMicrounits === 'number') {
      totalCostMicrounits += planningTurn.costMicrounits;
      sawAnyCost = true;
    }

    const parsedPlan = parsePlan(planningTurn.text);
    if (parsedPlan) {
      plan = parsedPlan;
    ...
    agentHistory.push({
      role: 'system',
      content: `Invalid protocol. Respond with ${PLAN_PREFIX} {"steps":[...]}.`
    });
  }

  if (!plan) {
    throw new Error('Agent failed to produce a valid execution plan.');
  }
  ...
  for (let step = 0; step < MAX_TOOL_STEPS; step += 1) {
    ...
    const modelTurn = await input.runCompletion({
      history: agentHistory,
      signal: input.signal,
      onEvent: (event) => {
    ...
    if (typeof modelTurn.costMicrounits === 'number') {
      totalCostMicrounits += modelTurn.costMicrounits;
      sawAnyCost = true;
    }

    const stepDone = parseStepDone(modelTurn.text);
    if (stepDone) {
      if (stepDone.index > plan.steps.length) {
        agentHistory.push({ role: 'assistant', content: modelTurn.text });
        agentHistory.push({
          role: 'system',
          content: `Invalid ${STEP_DONE_PREFIX} index ${stepDone.index}. Use 1..${plan.steps.length}.`
        });
        continue;
      }
    ...
    const parsedFinal = parseFinal(modelTurn.text);
    if (parsedFinal) {
    ...
    const toolCall = parseToolCall(modelTurn.text);
    if (toolCall) {
      input.onEvent?.({
        type: 'trace',
        trace: {
    ...
    agentHistory.push({
      role: 'system',
      content: `Invalid protocol response. Use ${TOOL_CALL_PREFIX}, ${STEP_DONE_PREFIX}, or ${FINAL_PREFIX}.`
    });
  }

  if (!finalResponse || !finalResponse.trim()) {
    throw new Error(`Agent reached max tool steps (${MAX_TOOL_STEPS}) without finishing.`);
  }
  ...
  return {
    ...
    costMicrounits: sawAnyCost ? totalCostMicrounits : undefined
  };
};
